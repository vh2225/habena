...
### Added
- **`agentguard security audit`** — static analysis over the resolved policy. Flags `wildcard-before-specific` (unreachable specific deny behind a wildcard allow), `orphan-server-reference` (rule matches a server not in `mcp_servers:`), and `host-policy-overridden-attempt` (user allow hidden under a host-policy deny). `--json` for CI gating; exit code = number of error-severity findings. First slice of Phase 9 V2.
